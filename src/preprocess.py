"""
Ce script applique un preprocessing aux fichiers files/train.txt et
files/test.txt.

Le script requiert les embeddings basés sur les dépendances de Levy et ses
compères, disponibles à l'adresse écrite dans le rapport et le fichier
corpus/README.md. Le chemin à cette dépendance doit être spécifié dans la
variable 'embeddingsPath'
"""
...
# Change the path here to the correct path for the embeddings file
...
def createMatrices(file, word2Idx, maxSentenceLen=100):
    ...
    for line in open(file):
        ...
        for idx in np.arange(0, min(maxSentenceLen, len(tokens))):
